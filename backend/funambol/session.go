...
// calls.  X-deviceid is already set as a default header on the client.
...
func Config(ctx context.Context, name string, m configmap.Mapper, configIn fs.ConfigIn) (*fs.ConfigOut, error) {
	...
	if f.opt.DeviceID == "" {
		return fs.ConfigError("", "device_id must be set before continuing (see its help for how to obtain it)")
	}
